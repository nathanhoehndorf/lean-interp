...
def test_feature_correlations(labels: list[dict]):
...
def build_cache_filename(config: ProbeConfig, layers: list[int]) -> Path:
    ...
    return Path(f"activations_cache_{config.mode.value}_{model_name_safe}_layers{layer_suffix}.pt")
...
def main() -> None:
    ...
    test_feature_correlations(train_labels)
